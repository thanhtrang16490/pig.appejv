...
import { Farm, Customer, ProductType, Transaction } from "@/lib/types/database";
...
export default function EditSalePage({ params }: EditSalePageProps) {
  ...
  // Fetch transaction data and reference data
  useEffect(() => {
    if (!transactionId) return;

    async function fetchData() {
      try {
        const [transactionResult, farmsResult, customersResult] = await Promise.all([
          supabase
            .from("transactions")
            .select("*")
            .eq("id", transactionId)
        ...
        const transaction = transactionResult.data as Transaction;
        ...
        setFormData({
          ...
          total_weight_kg: transaction.total_weight_kg.toString(),
          unit_price: transaction.unit_price.toString(),
          excess_price_per_kg: transaction.excess_price_per_kg.toString(),
          payment_cash: transaction.payment_cash.toString(),
          payment_bank: transaction.payment_bank.toString(),
          payment_company: transaction.payment_company.toString(),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/sales/${transactionId}`);
      router.refresh();
    } catch (error) {
      console.error("Error updating transaction:", error);
      setErrors({ submit: "Có lỗi xảy ra khi cập nhật đơn hàng. Vui lòng thử lại." });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div>
      ...
      <PageHeader title="Chỉnh sửa đơn hàng" />
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex gap-3">
          ...
          >
            {submitting ? "Đang lưu..." : "Cập nhật đơn hàng"}
          </button>
        </div>
      </form>
    </div>
